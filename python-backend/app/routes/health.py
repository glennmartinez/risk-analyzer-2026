...
async def health_check():
    ...
    return HealthResponse(
        status=status, version=settings.app_version, services=services
    )


@router.get("/", summary="Root endpoint", description="Welcome message and API info")
async def root():
...
@router.get(
    "/live", summary="Liveness check", description="Check if the service is alive"
)
async def liveness():
